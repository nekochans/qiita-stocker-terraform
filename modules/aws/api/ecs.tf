...
data "template_file" "user_data" {
  ...
  vars = {
    cluster_name = aws_ecs_cluster.api_ecs_cluster[0].name
  }
}
...
resource "aws_instance" "ecs_instance" {
  ...
  vpc_security_group_ids = [aws_security_group.ecs_api[0].id]
  key_name               = var.bastion["key_pair_id"]
...
resource "aws_ecs_service" "api_ecs_service" {
  count           = terraform.workspace != "prod" ? 1 : 0
  name            = lookup(var.ecs, "${terraform.workspace}.name", var.ecs["default.name"])
  cluster         = aws_ecs_cluster.api_ecs_cluster[0].id
